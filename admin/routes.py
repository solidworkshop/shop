...
from utils.events import send_capi_event, graph_url, get_pixel_id, get_test_event_code, should_attach_margin, should_attach_pltv, profit_margin
...
def dashboard():
    ...
    return render_template("admin/dashboard.html",
        running=running, counters=counters_snapshot(),
...
@login_required
def admin_health():
    return {"ok": True, "build": "v2.0.2", "graph": graph_url("")}, 200

@admin_bp.route("/inspector")
@login_required
def inspector():
    last = EventLog.query.order_by(EventLog.ts.desc()).limit(50).all()
    return render_template("admin/inspector.html", rows=last, build="v2.0.2")

@admin_bp.route("/logs")
@login_required
def logs_view():
    last = EventLog.query.order_by(EventLog.ts.desc()).limit(200).all()
    return render_template("admin/logs.html", rows=last, build="v2.0.2")
...
def admin_catalog():
    ...
    return render_template("admin/catalog.html", products=products, build="v2.0.2")
